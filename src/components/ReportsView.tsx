...
export const ReportsView: React.FC<ReportsViewProps> = ({ pageId, onNavigate, onUpdateData, showToast, appData }) => {
  ...
  const [draftFromDate, setDraftFromDate] = useState(monthAgoStr);
  ...
  const handleExportExcel = () => {
    try {
      ...
      exportToExcel({
        filename: `${groupKey}_${reportName}_${new Date().toISOString().split('T')[0]}`,
        sheetName: reportData.title || reportName || 'التقرير',
        data: exportRows,
        columns: excelColumns,
        companyName: appData.settings?.companyName || 'المنظومة المحاسبية المعتمدة',
        reportTitle: `${reportData.title || reportName} (من ${fromDate} إلى ${toDate})`,
      });
      ...
      link.download = `${groupKey}_${reportName}_${new Date().toISOString().split('T')[0]}.csv`;
      link.click();
  ...
  const handlePrintReport = () => {
    ...
    printReportWindow(reportData, appData, fromDate, toDate, showToast);
  };
    ...
    if (printableEl) {
      ...
      try {
        await exportElementToPdf(
          printableEl,
          `${reportData.title || reportName}_${fromDate}_${toDate}.pdf`,
          {
